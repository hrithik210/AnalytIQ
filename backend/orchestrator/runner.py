...
async def start(csv : str ):
	...
	vm.store_schema(report_id, interpreter_dict)
	...
	vm.store_wrangler(report_id, wrangler_output)
		
	df = pd.read_csv(cleaned_csv_path)
	
	df_sample = df.head(5).to_dict()

	# retrieve relevant context for downstream agents
	retrieved_schema_ctx = vm.retrieve_interpreter_context("dataset schema and suggested analysis", n_results=4)
	retrieved_wrangler_ctx = vm.retrieve_wrangler_context("data cleaning steps and audit log", n_results=4)

	# analyst
	analyst = Analyst()
	analyst_res = await analyst.run_analysis(
		cleaned_csv_path,
		interpreter_dict,
		wrangler_output['wrangling_report'],
		retrieved_context={
			"schema": retrieved_schema_ctx,
			"wrangler": retrieved_wrangler_ctx
		}
	)
	
	analyst_output = analyst_res.model_dump()
	print("analyst_output:", analyst_output)
	vm.store_analyst(report_id, analyst_output)

	# visualizer
	visualizer = Visualizer()
	# allow visualizer to use retrieved analyst context for RAG
	retrieved_analyst_ctx = vm.retrieve_analyst_context("key findings and correlations to visualize", n_results=4)
	visualization_output = await visualizer.create_visualization(cleaned_csv_path , analyst_output, retrieved_context={"analyst": retrieved_analyst_ctx})
	...
	vm.store_visualization(report_id, visualizer_res)

	qa_agent = QAAgent()
	# retrieve viz context for QA too
	retrieved_viz_ctx = vm.retrieve_visualizer_context("visualization recommendations and code", n_results=4)
	qa_response = await qa_agent.run_qa_review(interpreter_output=interpreter_dict,
						   ...
						   retrieved_context={
							   "schema": retrieved_schema_ctx,
							   "wrangler": retrieved_wrangler_ctx,
							   "analyst": retrieved_analyst_ctx,
							   "visualizer": retrieved_viz_ctx
						   }
						)
